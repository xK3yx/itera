...
function CourseCard({ course }) {
  ...
  return (
    <a
      ...
      className="block border border-gray-200 rounded-xl p-4 hover:border-blue-300 hover:shadow-sm transition bg-white"
    >
      <div className="flex items-start justify-between gap-2">
        <div className="flex-1">
          <p className="text-sm font-medium text-gray-800 leading-snug">{course.title}</p>
          {course.why_recommended && (
            <p className="text-xs text-gray-500 mt-1">{course.why_recommended}</p>
          )}
        </div>
        <span className="shrink-0 text-xs bg-blue-50 text-blue-600 font-medium px-2 py-1 rounded-lg">
          {course.platform}
        </span>
      </div>
      <div className="flex items-center gap-3 mt-3 text-xs text-gray-400">
        ...
        <span className="ml-auto text-blue-500 font-medium">Search course →</span>
      </div>
    </a>
  )
}
    ...
    <div className="border border-gray-200 rounded-xl overflow-hidden">
      ...
      >
        ...
        <div className="flex items-center gap-3 shrink-0">
          <span className="text-xs text-gray-400">{topic.estimated_hours}h</span>
      ...
      {open && (
        <div className="px-4 pb-4 bg-gray-50 border-t border-gray-100">
          {topic.description && (
            <p className="text-xs text-gray-500 mt-3 mb-3">{topic.description}</p>
          )}
          {topic.why_relevant && (
...
function SkillArea({ area, index }) {
  ...
  return (
    <div className="border border-gray-200 rounded-2xl overflow-hidden shadow-sm">
      ...
      {open && area.topics && area.topics.length > 0 && (
        <div className="px-5 pb-5 bg-gray-50 border-t border-gray-100 space-y-2 pt-3">
  ...
  return (
    <div className="mt-6 bg-gray-50 rounded-2xl p-5 border border-gray-200">
      <div className="mb-5">
        ...
        {roadmap.goal && (
          <p className="text-sm text-gray-500 mt-1">{roadmap.goal}</p>
        )}
      </div>

      <div className="grid grid-cols-3 gap-3 mb-5">
        <div className="bg-white rounded-xl p-3 text-center border border-gray-200">
          <p className="text-2xl font-bold text-blue-600">{roadmap.total_estimated_hours}</p>
          <p className="text-xs text-gray-500 mt-1">Total hours</p>
        </div>
        <div className="bg-white rounded-xl p-3 text-center border border-gray-200">
          <p className="text-2xl font-bold text-blue-600">{roadmap.weekly_hours}</p>
          <p className="text-xs text-gray-500 mt-1">Hours/week</p>
        </div>
        <div className="bg-white rounded-xl p-3 text-center border border-gray-200">
          <p className="text-2xl font-bold text-blue-600">{roadmap.estimated_weeks}</p>
